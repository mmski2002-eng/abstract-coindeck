...
import { HEROES, COIN_ICONS, TIER_NAMES, PLAYER_ROLE_IDS } from "../constants";
...
export function LineupConfirmModal({ lang, tnState, epochRange, roleBonusPct, tnSelectedCards, flCards, busy, setLineupConfirmOpen, onSubmitLineup }: Props) {
  return createPortal(
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4" style={{ background: "var(--overlay-backdrop)" }}>
      <div className="card-sticker w-full max-w-sm rounded-2xl p-5 space-y-4" style={{ background: "var(--modal-bg)" }}>
        ...
        <div className="rounded-xl px-3 py-2 text-[11px]" style={{ background: "var(--warn-soft)", border: "1.5px solid var(--outline)", color: "var(--ink-2)" }}>
          ⚡ {lang === "ru"
            ...
            return (
              <div
                ...
                style={{
                  ...
                  background: isRoleMatch ? "var(--warn-soft)" : "var(--panel-bg)",
                }}
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-1.5 mt-0.5">
                    <span className="chip-sticker text-[9px] rounded px-1 py-0.5 font-bold" style={{ boxShadow: "none" }}>{TIER_NAMES[card.tier]}</span>
                    <span className="text-[9px]" style={{ color: "var(--ink-3)" }}>{role}</span>
                  </div>
                </div>
                <span className="text-[10px] font-bold shrink-0" style={{ color: isRoleMatch ? "var(--warn)" : "var(--ink-3)" }}>
        ...
        <div className="flex gap-2 pt-1">
          <button
            ...
            className="btn-sticker-outline flex-1 rounded-xl py-2.5 text-xs font-semibold transition">
            {lang === "ru" ? "Назад" : "Back"}
          </button>
          <button
            onClick={async () => { setLineupConfirmOpen(false); await onSubmitLineup(); }}
            disabled={busy !== null}
            className="btn-sticker-primary flex-1 rounded-xl py-2.5 text-xs font-bold disabled:opacity-50 transition">
