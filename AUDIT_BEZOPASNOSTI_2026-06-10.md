...
|---|----------|----------|--------|
| C1 | CRITICAL | SSH root-пароль в `CLAUDE.md` (в git) | ✅ убран из CLAUDE.md (вынесен в `DEPLOY_CREDENTIALS.local.md`, gitignored), история git переписана и force-push в origin/main |
| C2 | CRITICAL | Прод-секреты в `frontend/doc_*.env.local` (в git) | ✅ файл удалён из git и из всей истории (force-push) |
| C3 | CRITICAL | Слабые секреты (ADMIN_SECRET/CRON_SECRET/DB pass) | ⏳ не сделано — требует синхронной смены значений в `.env.local` на сервере + рестарт + смена пароля Postgres (отдельная задача) |
| H1 | HIGH | Нет HSTS | ✅ добавлен `Strict-Transport-Security` в nginx на проде, проверено live |
| H2 | HIGH | CSP `unsafe-inline` для script-src | ⏳ пропущено по решению — требует nonce-based CSP (риск сломать inline-скрипты/стили) |
| M1 | MEDIUM | `/api/bot` GET без авторизации | ✅ из ответа убраны `txHashes` и адрес кошелька бота |
| L1 | LOW | `X-Powered-By: Next.js` | ✅ `poweredByHeader: false` в `next.config.ts` |
| L2 | LOW | Топология сервера в `CLAUDE.md` | ✅ вынесена в `DEPLOY_CREDENTIALS.local.md` |

**Важно:** SSH-пароль и прод-секреты были опубликованы на GitHub, поэтому считаются скомпрометированными независимо от чистки истории — рекомендуется сменить пароль root на сервере и ротировать ADMIN_SECRET/CRON_SECRET/MARKET_DATA_SECRET/DB-пароль (см. C3).
